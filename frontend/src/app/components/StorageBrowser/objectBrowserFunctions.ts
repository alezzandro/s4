  ...
  apiClient
    ...
    .then((response) => {
      if (response.status === 200) {
        ...
        notifyWarning('Error fetching buckets', 'Failed to fetch buckets from the backend.');
      }
    })
    .catch((error) => {
      console.error(error);
      notifyWarning(
        error.response?.data?.error || 'Error Fetching Buckets',
        error.response?.data?.message || 'Failed to fetch buckets from the backend.',
      );
    });
};
...
export const refreshObjects = (
  ...
  if (!prefix || prefix === ':prefix') {
    ...
    url = `/objects/${bucketName}/${encoded}`;
  }
  ...
  return apiClient
    ...
    .catch((error) => {
      ...
      if (!axios.isCancel(error)) {
        ...
        notifyWarning(
          error.response?.data?.error || 'Error Fetching Objects',
          error.response?.data?.message || 'Failed to fetch objects from the backend.',
        );
      }
    })
    .finally(() => {
...
export const uploadSingleFile = async (
  ...
  apiClient
    .post(`/objects/upload/${bucketName}/${encodedKey}`, formData, {
      headers: {
...
export const deleteFile = (
  ...
  apiClient
    ...
    .catch((error) => {
      ...
      notifyWarning(
        error.response?.data?.error || 'File Deletion Failed',
        error.response?.data?.message || String(error),
      );
    });
};
...
export const deleteFolder = (
  ...
  apiClient
    ...
    .catch((error) => {
      ...
      notifyWarning(
        error.response?.data?.error || 'Folder Deletion Failed',
        error.response?.data?.message || String(error),
      );
    });
};
...
export const createFolder = (
  ...
  apiClient
    ...
    .catch((error) => {
      ...
      notifyWarning(
        error.response?.data?.error || 'Folder Creation Failed',
        error.response?.data?.message || String(error),
      );
    });
};

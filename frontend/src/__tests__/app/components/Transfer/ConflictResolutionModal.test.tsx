...
describe('ConflictResolutionModal - Smart Conflict UI', () => {
  ...
  it('should show non-conflicting files info', () => {
    ...
    expect(screen.getByText(/2 file\(s\) will be copied automatically \(no conflicts\)/i)).toBeInTheDocument();
  });
  ...
  it('should show conflicting files count', () => {
    ...
    expect(screen.getByText(/3 file\(s\) already exist in the destination/i)).toBeInTheDocument();
  });
  ...
  it('should update summary based on resolution choice - skip', () => {
    ...
    expect(screen.getByText(/2 conflict\(s\) will be skip/i)).toBeInTheDocument();
  });
  ...
  it('should update summary based on resolution choice - overwrite', async () => {
    ...
    expect(screen.getByText(/2 conflict\(s\) will be overwrite/i)).toBeInTheDocument();
  });
  ...
  it('should update summary based on resolution choice - rename', async () => {
    ...
    expect(screen.getByText(/Total: 3 files will be copied/i)).toBeInTheDocument();
    expect(screen.getByText(/2 conflict\(s\) will be rename/i)).toBeInTheDocument();
  });
  ...
  it('should handle single file conflict correctly', () => {
    ...
    // Using file(s) format from i18n
    expect(screen.getByText(/1 file\(s\) will be copied automatically \(no conflicts\)/i)).toBeInTheDocument();
    expect(screen.getByText(/1 file\(s\) already exist in the destination/i)).toBeInTheDocument();
  });
  ...
  it('should handle multiple files conflict correctly', () => {
    ...
    // Using file(s) format from i18n
    expect(screen.getByText(/2 file\(s\) will be copied automatically \(no conflicts\)/i)).toBeInTheDocument();
    expect(screen.getByText(/2 file\(s\) already exist in the destination/i)).toBeInTheDocument();
  });
});

...
export async function listS3DirectoryRecursive(
  ...
  try {
    ...
    if (err instanceof S3ServiceException) {
      throw new ListingError(`S3 listing failed for bucket "${bucket}", prefix "${normalizedPrefix}": ${err.message}`);
    }
 ...
 * @throws ListingError if listing operation fails
 */
export async function listLocalDirectoryRecursive(basePath: string, relativePath: string): Promise<DirectoryListing> {
  const files: FileInfo[] = [];
  const emptyDirectories: string[] = [];

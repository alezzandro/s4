    ...
    try {
      ...
      if (logger) {
        ...
        if (error.code === 'ENOENT') {
          ...
          logger.warn(errorDetails, `Local storage path permission denied: ${dirPath} - Check directory permissions`);
        } else {
          logger.warn(errorDetails, `Local storage path not accessible: ${dirPath} - ${error.message}`);
        }
      }
    }

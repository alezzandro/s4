...
export function logMemory(label: string): MemorySnapshot {
  ...
  if (lastSnapshot) {
    ...
    deltaStr = ` | Δ RSS: ${rssDelta >= 0 ? '+' : ''}${formatBytes(
      rssDelta,
    )} | Δ Heap: ${heapDelta >= 0 ? '+' : ''}${formatBytes(heapDelta)} | Δ Time: ${timeDelta}ms`;
  }

  console.log(
    `[Memory] ${label.padEnd(40)} | RSS: ${formatBytes(snapshot.rss).padStart(
      9,
    )} | Heap: ${formatBytes(snapshot.heapUsed).padStart(9)}/${formatBytes(snapshot.heapTotal).padStart(
      9,
    )} | External: ${formatBytes(snapshot.external).padStart(9)}${deltaStr}`,
  );

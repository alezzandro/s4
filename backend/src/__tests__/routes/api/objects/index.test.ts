...
jest.mock('../../../../utils/rateLimit', () => ({
  ...
  getRateLimitResetTime: jest.fn().mockReturnValue(0),
}));
...
// Note: Upload is mocked above, no need to import it here
...
describe('Object Routes', () => {
  ...
  describe('GET /view/:bucketName/:encodedKey', () => {
    ...
    it('should handle other errors when viewing an object', async () => {
      ...
      const encodedKey = Buffer.from(key).toString('base64');
      s3Mock.on(GetObjectCommand, { Bucket: 'test-bucket', Key: key }).rejects(new Error('Some other view error'));
  ...
  describe('GET /download/:bucketName/:encodedKey', () => {
    ...
    it('should handle other errors when downloading an object', async () => {
      ...
      const encodedKey = Buffer.from(key).toString('base64');
      s3Mock.on(GetObjectCommand, { Bucket: 'test-bucket', Key: key }).rejects(new Error('Some other download error'));
  ...
  describe('DELETE /:bucketName/:encodedKey', () => {
    it('should delete a single object successfully', async () => {
      ...
      // Mock ListObjectsV2Command to return empty - triggers single object delete path
      s3Mock.on(ListObjectsV2Command, { Bucket: 'test-bucket', Prefix: key }).resolves({
        Contents: [], // Empty contents triggers DeleteObjectCommand
      });
      ...
      expect(response.statusCode).toBe(200);
      const payload = JSON.parse(response.payload);
      expect(payload.message).toBe('Object deleted successfully');
    });
    ...
    it('should delete objects under a prefix successfully', async () => {
      ...
      s3Mock
        .on(DeleteObjectsCommand, {
          Bucket: 'test-bucket',
          Delete: {
            Objects: objectsInPrefix.map((item) => ({ Key: item.Key })),
          },
        })
        .resolves({});
      ...
      expect(response.statusCode).toBe(200);
      const payload = JSON.parse(response.payload);
      expect(payload.message).toBe('Successfully deleted 2 object(s)');
    });

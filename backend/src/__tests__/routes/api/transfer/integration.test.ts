...
describe('Folder Copy Integration Tests', () => {
  ...
  const createLocalStructure = async (baseDir: string, structure: Record<string, string | null>): Promise<void> => {
  ...
  describe('T-2: Local Directory Listing with Empty Dirs and Symlinks', () => {
    ...
    it('should skip symbolic links and track them', async () => {
      ...
      // Create symlink
      await createSymlink(path.join(testDir, 'data/target.txt'), path.join(testDir, 'data/link.txt'));
  ...
  describe('T-5: Mixed Scenarios', () => {
    it('should handle mixed files, empty dirs, and symlinks', async () => {
      ...
      // Add symlink
      await createSymlink(path.join(testDir, 'data/file1.txt'), path.join(testDir, 'data/link.txt'));

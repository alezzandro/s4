...
jest.mock('../../../../utils/transferQueue', () => ({
  transferQueue: {
    ...
    // Return a function that just executes the passed callback directly
    getMetadataLimiter: jest.fn().mockReturnValue((fn: () => Promise<any>) => fn()),
  },
}));
...
describe('Empty Directory Handling (.s3keep markers)', () => {
  ...
  describe('.s3keep marker behavior', () => {
    ...
    it('should detect .s3keep by basename', () => {
      const paths = ['data/empty/.s3keep', 'project/models/.s3keep', 'nested/path/to/empty/.s3keep'];
  ...
  describe('Round-trip directory preservation', () => {
    it('should preserve empty directories through Local→S3→Local', async () => {
      ...
      expect(await fs.readFile(path.join(destDir, 'project', 'src', 'main.py'), 'utf-8')).toBe('code');
    });
  ...
  describe('Edge cases', () => {
    ...
    it('should handle nested structure with mixed empty/full directories', async () => {
      ...
      await fs.mkdir(path.join(baseDir, 'nested', 'empty2'), {
        recursive: true,
      });
...
describe('POST /check-conflicts - Updated Endpoint (Task 1.7)', () => {
  ...
  beforeEach(async () => {
    ...
    (validatePath as jest.Mock).mockImplementation(async (_locationId: string, filePath: string) => {
  ...
  describe('Large Folder Warning', () => {
    ...
    it('should warn when folder total size >= 10GB', async () => {
      ...
      expect(body.warning.message).toContain('11.00 GB');
    });

...
describe('Local Storage Utilities - Security Tests', () => {
  ...
  describe('validatePath - Path Traversal Protection', () => {
    it.each(PATH_TRAVERSAL_ATTACKS)('should reject path traversal attack: %s', async (attackPath) => {
    ...
    it('should reject invalid location ID format', async () => {
      ...
      await expect(validatePath('invalid-id', 'file.txt')).rejects.toThrow('Invalid location ID: invalid-id');
    });
    ...
    it('should reject out-of-bounds location index', async () => {
      ...
      await expect(validatePath('local-99', 'file.txt')).rejects.toThrow('Location index out of bounds: 99');
    });
    ...
    it('should reject absolute paths in relativePath', async () => {
      ...
      await expect(validatePath('local-0', '/etc/passwd')).rejects.toThrow('Absolute paths not allowed');
    });
    ...
    it('should throw NotFoundError for non-existent parent directory', async () => {
      await expect(validatePath('local-0', 'missing/deep/path/file.txt')).rejects.toThrow(NotFoundError);
    });
  ...
  describe('getStorageLocations', () => {
    ...
    it('should log warnings for unavailable paths', async () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.any(Object),
        expect.stringContaining('Local storage path does not exist: /mnt/missing'),
      );
    });
  ...
  describe('listDirectory', () => {
    ...
    it('should throw StorageError for ENOTDIR', async () => {
      ...
      await expect(listDirectory('/opt/app-root/src/data/file.txt')).rejects.toThrow('Not a directory');
    });
  });
  ...
  describe('createDirectory', () => {
    ...
    it('should throw PermissionError for EACCES', async () => {
      ...
      await expect(createDirectory('/opt/app-root/src/data/forbidden')).rejects.toThrow(PermissionError);
    ...
    it('should throw StorageError for ENOSPC', async () => {
      ...
      const { fs } = require('memfs');
      const originalMkdir = fs.promises.mkdir;
      fs.promises.mkdir = jest.fn().mockRejectedValue(
  ...
  describe('deleteFileOrDirectory', () => {
    ...
    it('should return correct count for directory', async () => {
      vol.mkdirSync('/opt/app-root/src/data/testdir/nested', {
        recursive: true,
      });
    ...
    it('should throw NotFoundError for non-existent path', async () => {
      await expect(deleteFileOrDirectory('/opt/app-root/src/data/missing.txt')).rejects.toThrow(NotFoundError);
    });
    ...
    it('should throw PermissionError for EACCES', async () => {
      ...
      fs.promises.stat = jest.fn().mockRejectedValue(Object.assign(new Error('Permission denied'), { code: 'EACCES' }));

      await expect(deleteFileOrDirectory('/opt/app-root/src/data/file.txt')).rejects.toThrow(PermissionError);
  ...
  describe('getFileMetadata', () => {
    ...
    it('should throw NotFoundError for non-existent file', async () => {
      await expect(getFileMetadata('/opt/app-root/src/data/missing.txt')).rejects.toThrow(NotFoundError);
    });
    ...
    it('should throw PermissionError for EACCES', async () => {
      ...
      await expect(getFileMetadata('/opt/app-root/src/data/file.txt')).rejects.toThrow(PermissionError);
  ...
  describe('checkFileSize', () => {
    ...
    it('should throw StorageError for file exceeding limit', async () => {
      ...
      await expect(checkFileSize('/opt/app-root/src/data/large.bin')).rejects.toThrow(/exceeds limit/);
    ...
    it('should throw NotFoundError for non-existent file', async () => {
      await expect(checkFileSize('/opt/app-root/src/data/missing.txt')).rejects.toThrow(NotFoundError);
    });

...
describe('Directory Listing Utilities', () => {
  describe('listS3DirectoryRecursive', () => {
    ...
    it('should handle S3ServiceException errors', async () => {
      ...
      await expect(listS3DirectoryRecursive(mockS3Client, 'test-bucket', 'data')).rejects.toThrow(ListingError);
    });
  ...
  describe('listLocalDirectoryRecursive', () => {
    ...
    it('should list all files in flat directory', async () => {
      ...
      expect(result.files).toHaveLength(3);
      expect(result.fileCount).toBe(3);
      expect(result.files.map((f) => f.path).sort()).toEqual(['file1.txt', 'file2.txt', 'file3.bin'].sort());
      expect(result.emptyDirectories).toEqual([]);
      expect(result.skippedSymlinks).toEqual([]);
    });
    ...
    it('should handle permission errors', async () => {
      ...
      await expect(listLocalDirectoryRecursive('/data', 'restricted')).rejects.toThrow(ListingError);
      await expect(listLocalDirectoryRecursive('/data', 'restricted')).rejects.toThrow(/Permission denied/);
    });

    it('should handle non-existent directory', async () => {
      vol.fromJSON({});

      await expect(listLocalDirectoryRecursive('/data', 'nonexistent')).rejects.toThrow(ListingError);
      await expect(listLocalDirectoryRecursive('/data', 'nonexistent')).rejects.toThrow(/not found/);
    });

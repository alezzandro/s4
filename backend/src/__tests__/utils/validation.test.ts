...
describe('validateContinuationToken', () => {
  ...
  describe('invalid tokens - format violations', () => {
    it('should reject tokens with spaces', () => {
      expect(validateContinuationToken('token with spaces')).toBe('Continuation token format is invalid.');
    });

    it('should reject tokens with special characters', () => {
      expect(validateContinuationToken('token<script>')).toBe('Continuation token format is invalid.');
      expect(validateContinuationToken('token@value')).toBe('Continuation token format is invalid.');
      expect(validateContinuationToken('token#value')).toBe('Continuation token format is invalid.');
      expect(validateContinuationToken('token$value')).toBe('Continuation token format is invalid.');
    });
  });
...
describe('validateAndDecodePrefix', () => {
  ...
  describe('invalid prefixes - path traversal', () => {
    it('should reject path traversal sequences', () => {
      const pathTraversalAttacks = ['../../etc/passwd', '../../../root', 'folder/../../etc', 'test/../../../passwd'];

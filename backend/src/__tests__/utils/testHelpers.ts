 ...
 */
export function createMockFilesystem(files: Record<string, string | Buffer> = {}): typeof vol {
  vol.reset();
  vol.fromJSON(files);
  return vol;
}
 ...
 */
export function createTestDirectoryStructure(): Record<
  string,
  Record<string, string | Buffer | Record<string, string>>
> {
  return {
